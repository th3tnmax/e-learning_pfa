...
//chat authentication
router.post("/chatauth", async (req, res) => {
    const data = req.body;
    try {
        const r = await axios.put(
          "https://api.chatengine.io/users/",
          { username: data.name, secret: data.password, first_name: data.name ,last_name:data.lastname },
          { headers: { "Private-Key": "fd91a3a5-add2-4684-976e-c85fa8e5be1a" } }
        );
